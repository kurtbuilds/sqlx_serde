...
use sqlx_core::postgres::PgValueRef; // 1.0.104
        ...
        S: Serializer,
{
    ...
    match name {
        "BOOL" => {
            let v = bool::decode(value).unwrap();
            s.serialize_bool(v)
        }
        "INT2" => {
            let v = i16::decode(value).unwrap();
            s.serialize_i16(v)
        }
        "INT4" => {
            let v = i32::decode(value).unwrap();
            s.serialize_i32(v)
        }
        "INT8" => {
            let v = i64::decode(value).unwrap();
            s.serialize_i64(v)
        }
        "FLOAT4" => {
            let v = f32::decode(value).unwrap();
            s.serialize_f32(v)
        }
        "FLOAT8" | "NUMERIC" => {
            let v = f64::decode(value).unwrap();
            s.serialize_f64(v)
        }
        "CHAR" | "VARCHAR" | "TEXT" | "\"CHAR\"" => {
            let v = String::decode(value).unwrap();
            s.serialize_str(&v)
        }
        "BYTEA" => {
            let v = Vec::<u8>::decode(value).unwrap();
            s.serialize_some(&v)
        }
        "JSON" | "JSONB" => {
            let v = Value::decode(value).unwrap();
            s.serialize_some(&v)
        }
        "TIMESTAMP" => {
            let v = sqlx::types::chrono::NaiveDateTime::decode(value).unwrap();
            let v = v.format("%Y-%m-%dT%H:%M:%S.%f").to_string();
            s.serialize_str(&v)
        }
        "TIMESTAMPTZ" => {
            use sqlx::types::chrono;
            let v = chrono::DateTime::<chrono::Utc>::decode(value).unwrap();
